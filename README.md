...
**Format support note**: this project was originally built and tuned around Sony `.ARW` output specifically. `.NEF`/`.DNG`/`.RAF` support (added after V8) means the pipeline *accepts and processes* those formats the same way — each has been run end-to-end through RawTherapee with real sample files (Nikon, Fujifilm X-Trans, Leica) and produces a valid image. Colour tuning per sensor is only partly done: `profiles/default.pp3`/`profiles/lowlight.pp3` and the 32 look presets were dialled in and visually verified against Sony sensor output only. The one measured correction so far is [`profiles/raf.pp3`](profiles/raf.pp3) — a white-balance override for Fujifilm X-Trans, where the base profile's auto-WB left a cool cast on a test shot; it auto-stacks on `.raf` files. Nikon `.NEF` and Leica `.DNG` samples rendered neutral on the base profile and get no override. This is all based on one sample per format, so on a non-Sony body treat every profile and preset (including `raf.pp3`) as a starting point and **preview before trusting a look for a shoot** (see "Choosing a preset for a shoot" in the Presets section below — the same "preview first" advice already given there for scene type). More format-specific profiles may be added in a future version.
...
profiles/*.pp3          RawTherapee color-correction profiles (named, selectable, ISO-branched); profiles/<ext>.pp3 (e.g. raf.pp3) auto-stacks a per-format correction on files of that extension
...
3. **Per-photo profile selection**: if `autoProfile.enabled` is `true` in the config and `exiftoolPath` points at a working [ExifTool](https://exiftool.org/) install, each file's ISO is read and used to pick between `lowIsoProfile` and `highIsoProfile` (threshold: `isoThreshold`). Pass `-Profile <name>` or `-ProfilePath <file.pp3>` explicitly to bypass this and force one profile for the whole run. Separately, if a `profiles/<ext>.pp3` exists for a file's extension (currently just `profiles/raf.pp3`, an X-Trans white-balance correction), it is stacked as a thin overlay on top of the chosen base profile — `rawtherapee-cli -p <base> -p <format-overlay> [-p <preset>]` — so per-format fixes don't require touching the base profiles. This is not an ISO/`-Profile` override; it always applies to that extension.
...
These were verified for correct RawTherapee syntax, checked for measurable difference from the base and from each other, and visually inspected — but the reference photo used during development was a daytime indoor car-show shot **taken on a Sony body**, so (a) effects tuned for foliage/skin/neon-lit night scenes (`nature_earth`, `moody_warm`, `neon_nights`, etc.) will read more strongly on photos with that actual content, and (b) none of the presets have been re-checked against `.NEF`/`.DNG`/`.RAF` output — a different sensor's colour science is another axis of "your mileage may vary". Run `preview_presets.ps1` against a real photo from the relevant scenario (and from the actual camera) before trusting a look for a shoot.
...
**Per-format overlays**: a file named `profiles/<ext>.pp3` (e.g. `profiles/raf.pp3`) is automatically stacked *on top of* whichever base profile is chosen, for any file with that extension. Keep it thin — only the sections that need a per-sensor correction, so everything else stays inherited from the base. The one shipped example, `profiles/raf.pp3`, overrides just white balance to `Camera` for Fujifilm X-Trans files, where the base profile's auto-WB (tuned on Sony) measured a cool cast on a test shot. It's a measured starting point from a single sample — refine it against your own files with `preview_presets.ps1` (which applies the overlay too) before trusting it for a shoot.
...
```
`Import-Module ... -Force` first is needed because of the old inbox copy above - without it, PowerShell may resolve `Invoke-Pester` to Pester 3.4 instead of the one just installed. As of this writing: 24 tests, all green (covering `auto_enhance.ps1`'s pipeline behavior including multi-format `.nef`/`.dng`/`.raf` handling and the per-format profile overlay, and `setup.ps1`'s ExifTool auto-discovery). `preview_presets.ps1`'s own RawTherapee invocation isn't covered yet - it uses a deliberately different `Start-Process`-based pattern (to run several preview renders concurrently) rather than `auto_enhance.ps1`'s synchronous call, which the current wrapper-function setup doesn't reach.

**Manual/pipeline verification** — each capability below was verified against real sample `.ARW` files during development. The core conversion path (fresh run, idempotent re-run skip, output naming, a real `rawtherapee-cli` render producing a valid image) was additionally re-run end-to-end against real `.NEF` (Nikon D200), `.RAF` (Fujifilm X-T2, X-Trans) and `.DNG` (Leica M9) sample files when multi-format support landed; the format-specific automated tests cover discovery/routing/security. The remaining items below (quarantine, watch-folder timing, per-preset visual inspection, full browser run) were not separately re-run per new format:
