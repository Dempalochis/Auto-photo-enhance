...
**Format support note**: this project was originally built and tuned around Sony `.ARW` output specifically. `.NEF`/`.DNG`/`.RAF` support (added after V8) means the pipeline *accepts and processes* those formats the same way — each has been run end-to-end through RawTherapee with real sample files (Nikon, Fujifilm X-Trans, Leica) and produces a valid image. What has **not** been done is per-sensor colour tuning: `profiles/default.pp3`/`profiles/lowlight.pp3` and the 32 look presets were dialled in and visually verified against Sony sensor output only, and there are no format-specific profile variants yet. Different cameras' colour science can render differently through the same profile, so on a non-Sony body treat every profile and preset as a starting point and **preview before trusting a look for a shoot** (see "Choosing a preset for a shoot" in the Presets section below — the same "preview first" advice already given there for scene type). Format-specific profiles may be added in a future version.
